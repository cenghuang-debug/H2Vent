# Linux headless version for 2 m³ cases.
# Adapted from paraview_script_1m3_linux.py — portrait aspect ratio for 2 m tall enclosure.
#
# Usage:
#   pvpython paraview_script_2m3_linux.py <case_name> [--mode H2_vol_con|U] [--debug]
#   e.g.:
#   pvpython paraview_script_2m3_linux.py 2m3_27mm_wall_H2_test_56 --mode H2_vol_con
...
DEBUG       = False
IMG_WIDTH   = 3000
IMG_HEIGHT  = 4000   # portrait — 2 m enclosure is twice as tall as 1 m

ENC_Y_MIN   = -1.5
ENC_Y_MAX   =  1.5
ENC_Z_MIN   =  0.0
ENC_Z_MAX   =  3.5   # full external domain height for 2 m³ cases
MARGIN_TOP  =  0.08
MARGIN_BOT  =  0.22
MARGIN_SIDE =  0.10
...
TEXT_SIZE   = 71    # same as 1m3_linux: 50 (PV5.12) × (55px/39px) ≈ 71 for PV5.10
SLICE_X     =  0.001
...
# (Matches paraview_script_1m3_linux.py — previously this used
# ApplyPreset('X Ray'), which in ParaView 5.10 is a plain white->black
# grayscale, not the intended yellow/red ramp.)
x_H2LUT.RGBPoints = [
    0.0, 1.0, 1.0, 1.0,
    0.5, 1.0, 1.0, 0.0,
    1.0, 1.0, 0.0, 0.0,
...
import glob
_room_stl_matches = glob.glob(os.path.join(BASE_PATH, args.case_name, 'constant', 'triSurface', 'box_shell*_m.stl'))
if _room_stl_matches:
    roomShell = STLReader(registrationName='RoomShell', FileNames=[_room_stl_matches[0]])
    ...
    print(f'[WARNING] room shell STL not found in {os.path.join(args.case_name, "constant", "triSurface")}, skipping outline')
