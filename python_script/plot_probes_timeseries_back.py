"""
plot_probes_timeseries_back.py — Plot vertical H2 probes (vol%) vs time using the
back-wall mast (opposite to vents), for comparison with experiment.

Sensors are on the wall opposite to the vents (y ≈ +0.45 m), matching the
experimental mast placement in Bernard-Michel & Houssin-Agbomson (2017), Fig. 2.

Usage:
    python3 plot_probes_timeseries_back.py <case_number>
    e.g.:
        python3 plot_probes_timeseries_back.py 42    # 2 m³, 4 mm
"""
...
CASE_PARAMS = {
    ...
    50: (  5.2,  4, 2),
    58: (73, 27, 2), 59: (20.8, 27, 2), 60: (5.2, 27, 2),
}
...
# ── main ─────────────────────────────────────────────────────────────────────
case_num = int(sys.argv[1]) if len(sys.argv) > 1 else 42
...
if not os.path.isdir(probe_dir):
    print(f"ERROR: postProcessing/probes_H2_vertical_back not found in {case_dir}")
    sys.exit(1)
...
ax.set_title(f"Case {case_num} — {vol_m3} m³, {nozzle_mm} mm nozzle, {q_str}\n"
             f"Back-wall probes (opposite to vents)")
ax.legend(fontsize=11, loc="upper left", bbox_to_anchor=(1.01, 1), borderaxespad=0)
ax.set_ylim(bottom=0)

plt.tight_layout(rect=[0, 0, 0.82, 1])
out_base = os.path.join(os.path.dirname(__file__), f"probes_timeseries_back_case{case_num:02d}")
